...
def preveripoteze(poteze):
          poteze+=1
def izberi_polje(poteze):
  try:
    s=int(input('Izberi potezo: ')) 
    print('\n')
    x=6
    while plosca[x][s-1] !='*':
      x-=1
    plosca[x][s-1]=crka ### x je vrstica s pa stolpc
    return True
  except IndexError:
...
def stolp_zmaga(crka): #zmaga v stolpcu
  for x in range(6,-1,-1):
    for y in range(7):
      try:
        if plosca[x][y]==plosca[x-1][y]==plosca[x-2][y]==plosca[x-3][y]==crka
          return True
        else:
          continue
      except IndexError:
...
def vrsta_zmaga(crka): ### zmaga v vrstici
  for x in range(6,-1,-1):
    for y in range(7):
      try:
        if plosca[x][y]==plosca[x][y+1] ==plosca[x][y+2]==plosca[x][y+3]==crka:
          return True
        else:
          continue
      except IndexError:
...
while True:
  ...
  if stolp_zmaga(crka):
    ...
    break
  elif vrsta_zmaga(crka):
    print(crka,'Je zmagal po vrstici!')
    break
  elif diag_zmaga(crka):
